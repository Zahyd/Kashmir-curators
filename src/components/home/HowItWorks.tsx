      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-24 animate-fade-up">
          <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm mb-6">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.3em] text-[#111439]">Operational Protocol</span>
          </div>
          <h2 className="font-display text-5xl md:text-7xl font-black text-[#111439] tracking-tight mb-6 uppercase">
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute -top-12 -left-12 text-[120px] font-black text-[#111439]/[0.05] select-none group-hover:text-amber-500/[0.1] transition-colors duration-700">
                  0{index + 1}
                </div>

                <div className="relative z-10">
                  <div className="w-20 h-20 bg-white border border-slate-200/90 rounded-2xl flex items-center justify-center mb-10 group-hover:border-amber-400 group-hover:bg-amber-50/50 transition-all duration-700 rotate-3 group-hover:rotate-0 shadow-lg shadow-slate-900/5">
                    <Icon className="w-8 h-8 text-amber-600" />
                  </div>
                  
                  <h4 className="font-display text-2xl font-black text-[#111439] mb-4 tracking-tight group-hover:text-amber-600 transition-colors duration-500">
                    {step.title}
                  </h4>
                  
                  <p className="text-[#4A5568] leading-relaxed font-medium transition-colors duration-500">
                    {step.description}
                  </p>

                  {/* Connector - Desktop Only */}
                  {index < steps.length - 1 && (
                    <div className="hidden lg:block absolute top-10 -right-20 w-12 h-[1px] bg-slate-200 group-hover:bg-amber-400/60 transition-colors duration-500" />
                  )}
                </div>
              </div>
            );
